...
function jqqrcode(){
    ...
    if(serialcode==''){
        ...
        $('#pageBind').remove();
        $page.find('.qrcode').attr('src',app_url+"/Institutions/logoqrcode?key="+serialcode+"&data="+app_url+"/register/"+group_name+"/sc/"+serialcode);
...
function Unbindwx(){
    ...
    if(serialcode==''){
        ...
        $page.find('.unbind_btn').bind('click',function(){
            ...
            $.MsgBox.Confirm('提示',"<div align=\"center\">确定要解除绑定吗？</div>",function(){
                $.ajax({
                    ...
                    success:function(data){
                        ...
                        if(data.retCode=="0"){
                            $.MsgBox.Tip('success',data.retText,function(){
                                window.location.href=app_url+"/qrcode/"+group_name;
                            });
                        }else{
                            $.MsgBox.Tip('error',data.retText);
                        }

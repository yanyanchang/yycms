...
$("#pageSignin").bind("init",function(){
    $('#signinForm').attr('action',api_path+'/Server/register');
    $("title").html("在线开户");
    var $page=$(this);
    $page.find('.step1').show();
        ...
        var options = {
            success: function(data) {
                ...
                if(data.retCode==0){
                    ...
                    $('#go_to_wp').bind('click',function(){
                        location.href=api_path+'?m=Wp&code='+loginCode;
                    })
                    $page.find('.step3').hide();
                    $page.find('.step4').show();
                }else{
...
function downLoadPhoto(userid,callBack){
    ...
    function download(){
        $.ajax({
            url:api_path+'/Server/downLoadWxPhoto',

(function() {
    ...
    var GenerateHtml = function(type, title, msg, callback) {
		...
		if (type == "photo") {
            ...
            //_html += '<button type="button" id="mb_btn_ok" style=" height:50px;font-size:14px;line-height:50px; float:left; text-align:center; width:100%;background:#498DDE; " class="bgcolor-EF8 btnbgimg color-FFF width-100p ok">确定</button>';
		...
		if (type == "Loading") {
			...
			var loadingTop=(($(window).height()-$('.spinner-box').height())/2)/initial_scale;
			var loadingLeft=(($(window).width()-$('.spinner-box').width())/2)/initial_scale;
